...
# Copyright 2009-2013 Johns Hopkins University (Author: Daniel Povey, Gaurav Kumar)
# Apache 2.0.

if [ $# -lt 2 ]; then
   echo "Arguments should be the location of the Spanish Fisher Speech and Transcript Directories, see ../run.sh for example."
   exit 1;
fi
...
fcount_d1=`ls -l ${speech_d1} | wc -l`
fcount_d2=`ls -l ${speech_d2} | wc -l`
fcount_t=`ls -l ${transcripts} | wc -l`
#TODO:Count Mismatch between index files and actual files
#TODO:In addition, it seems like not all speech files have transcripts
#Now check if we got all the files that we needed
if [ $fcount_d1 != 412 -o $fcount_d2 != 409 -o $fcount_t != 682 ];
then
	echo "Incorrect number of files in the data directories"
	echo "DISC1 and DISC2 should contain 412 and 409 .sph files respectively"
	echo "The transcripts should contain 682 files"
	exit 1;
fi
