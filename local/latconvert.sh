...
prunebeam=2
partition=callhome_train

stage=0

if [ -d exp/tri5a/decode_$partition ]
then
	...
	for l in exp/tri5a/decode_$partition/lat.*.gz
	do	
		(
		...
		if [ $stage -le 0 ]; then
            ...
            # Prune lattices
            $KALDI_ROOT/src/latbin/lattice-prune --acoustic-scale=0.077 --beam=$prunebeam ark:"$bname.raw" ark:"$bname.pruned"

            # Convert to an openfst compatible format
		 	$KALDI_ROOT/src/latbin/lattice-to-fst --lm-scale=1.0 --acoustic-scale=0.077 ark:$bname.pruned ark,t:$bname.ark.fst	
