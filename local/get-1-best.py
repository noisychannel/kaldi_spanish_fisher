...
def findTranscription(timeDetail):
	file1 = open('exp/tri5a/decode_dev/scoring/13.tra')
	file2 = open('exp/tri5a/decode_dev2/scoring/13.tra')
	for line in file1:
		lineComp = line.split()
...
if not os.path.exists('exp/tri5a/one-best/test'):
	os.makedirs('exp/tri5a/one-best/test')

#provFile = open('/export/a04/gkumar/corpora/fishcall/fisher_provisional_dev.es', 'w+')
provFile = open('/export/a04/gkumar/corpora/fishcall/jack-splits/split-matt/asr.dev2', 'w+')
for item in fileList:
	timingFile = open('/export/a04/gkumar/corpora/fishcall/fisher/tim/' + item + '.es')
	newFile = open('exp/tri5a/one-best/test/' + item + '.es', 'w+')
