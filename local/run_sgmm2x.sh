...
utils/mkgraph.sh data/lang exp/sgmm2x_6a exp/sgmm2x_6a/graph || exit 1;
...
steps/decode_sgmm2.sh --use-fmllr true --config conf/decode.config --nj 20 --cmd "$decode_cmd" \
 ...
 steps/align_sgmm2.sh --nj 8 --cmd "$train_cmd" --transform-dir exp/tri5a \
    --use-graphs true --use-gselect true data/train data/lang exp/sgmm2x_6a exp/sgmm2x_6a_ali || exit 1;
 steps/make_denlats_sgmm2.sh --nj 8 --sub-split 20 --cmd "$decode_cmd" --transform-dir exp/tri5a \
